             ...
             <nav style="display: flex; gap: 1.5em;">
              <div class="nav-dropdown">
                  ...
                  <div class="dropdown-content">
                      ...
                      <a href="pass_activities.html">Pass Activities</a>
...
foreach ($file in $htmlFiles) {
    ...
    # Check if the file already has the dropdown CSS link
    $hasDropdownCss = $content -match 'dropdown\.css'
    
    # Add dropdown CSS link if not present
    if (-not $hasDropdownCss) {
        # Find the position to insert the CSS link (after the last link or before </head>)
        $lastLinkPos = $content.LastIndexOf('</head>')
        if ($lastLinkPos -gt 0) {
            $cssLink = '    <link rel="stylesheet" href="styles/dropdown.css">' + "`n"
            $content = $content.Insert($lastLinkPos, $cssLink)
        }
    }
    
    # Replace the old header with the new one containing dropdown
    $oldHeaderPattern = '(?s)<header style="width: 100vw; background-color: #3e2723;.*?<\/div>\s*(?=<!-- Page Title -->)'
    $content = [regex]::Replace($content, $oldHeaderPattern, $headerWithDropdown)
...
Write-Host "All HTML files have been updated with the dropdown menu."
